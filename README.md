...
packages. You can either run the bundled installer or install manually:

```bash
# Non-interactive install without nginx/systemd using a virtualenv in /opt/rpicamserver/.venv
./install.sh --non-interactive --no-nginx --no-systemd --install-dir /opt/rpicamserver --from-local
...
The installer supports flags such as `--no-apt` (skip apt-get entirely),
`--no-nginx`, `--no-systemd`, `--repo`/`--branch` for cloning a different
remote, and `--install-dir` to override `/opt/rpicamserver`. It refuses to
overwrite non-git directories and will update an existing checkout instead of
clobbering it.
...
- Restart a camera: `curl -X POST http://<host>:8000/api/cameras/<id>/restart`
- Use the bundled helper: `python cli.py devices --no-probe-missing`, `python cli.py set cameras.json`, `python cli.py restart <id>`, or `python cli.py delete <id>`
...
- `DELETE /api/cameras/{id}` – Remove a camera from the configuration and restart capture threads
...
- Basic auth (disabled by default) protects `/settings`, `/api/cameras`, and `/api/devices`. Configure it from the Settings page or by adding an `auth` block to `cameras.json`. When auth is enabled, streams are protected by default via `protect_streams` (toggle in the UI if you prefer them open).
- Stream protection is opt-in only when auth itself is disabled; consider pairing with Nginx auth if you prefer proxy-level guards.
